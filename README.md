...
### 7.3. 정책 설정 (admin/config.admin.json, admin/config.dist.json)
...
### 7.4. Progressive Checking (적응형 체크 간격)
...
### 11.2. CloudWatch Logs 그룹 생성 및 로깅 활성화
...
> **경로 설정** (각자 환경에 맞게 한 번만 변경):
> ```bash
> SOURCE_REPO=/home/ubuntu/cost-guardrail-claude-code-bedrock  # 소스 저장소 경로
> DIST_REPO=/home/ubuntu/bedrock-cost-guardrail                # 마켓플레이스 저장소 경로
> ```

### 14.1. 현재 비용 확인

```bash
cd "$SOURCE_REPO" && bash hooks/check-cost.sh --event report 2>&1
```
...
```bash
# 소스 저장소로 이동 후 직원 임계값 변경
cd "$SOURCE_REPO"
vi admin/config.dist.json    # threshold_usd를 100으로 변경

# 마켓플레이스 저장소에 배포 & 커밋 & 푸시
bash scripts/release.sh "$DIST_REPO" && \
  cd "$DIST_REPO" && \
...
# 소스 저장소로 이동 후 임계값 복원
cd "$SOURCE_REPO"
vi admin/config.dist.json    # threshold_usd를 원래 값으로 변경
